...
library(raster)
library(keras)
#walk through this with one image
...
#skip for now - if this works overall, this step won't be hard to adjust

#####4. make into a list - the output of a generator should be a list

generated_list <- list(test_array, test_label)

#now combine these steps into the generator as outline above
#data: output of list.files(input_dir, recursive = T, full.names = T)
#don't need x and y - both are parsed from the same input data
#batch_size
...
#generator to take batches of tiffs and create tensors
tiff_generator <- function(data,batch_size) {
  ...
  function() {
    ...
    #to test, set i to 79 and batch size to 5. rows will be just 79 and 80
